...
LOCAL_PATH="/root/autodl-tmp/nash-math-7b"
LOG_DIR="/root/RAGEN/logs/nash-math-7b"
mkdir -p "$LOG_DIR" # 如果目录不存在，则创建它

# 获取当前时间，格式为 YYYY-MM-DD-HHMMSS
TIMESTAMP=$(date +"%m-%d-%H-%M")

WANDB_MODE=offline RAY_DEDUP_LOGS=0 python train.py --config-name _14_FI1_mixed \
