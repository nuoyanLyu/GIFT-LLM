...
LOCAL_PATH="/root/autodl-tmp/math-7b"
LOG_DIR="/root/RAGEN/logs/math-7b"
MODEL_PATH="/root/autodl-tmp/math-7b/math50"
mkdir -p "$LOG_DIR" # 如果目录不存在，则创建它

# 获取当前时间，格式为 YYYY-MM-DD-HHMMSS
TIMESTAMP=$(date +"%m-%d-%H-%M")

WANDB_MODE=offline RAY_DEDUP_LOGS=0 HYDRA_FULL_ERROR=1 python train.py --config-name _10_math \
 ...
 trainer.experiment_name=math-7b \
